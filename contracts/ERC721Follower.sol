...
// ERC721Follower Contracts v0.1.0
// Creator: Coroodles Dev

pragma solidity ^0.8.4;

import "./IERC721Follower.sol";
import "./utils/Ownable.sol";

/**
 * @dev ERC721 token receiver interface.
 */
interface ERC721Follower__IERC721Receiver {
 ...
 */
contract ERC721Follower is IERC721Follower, Ownable {
    ...
    // Divided FundAmount sender --> Divided FundAmount
    ...
    function approve(address to, uint256 tokenId) public override {
        ...
        if (_msgSenderERC721Follower() != owner)
            if (!isApprovedForAll(owner, _msgSenderERC721Follower())) {
    ...
    function setApprovalForAll(address operator, bool approved)
        ...
        override
    {
        if (operator == _msgSenderERC721Follower()) revert ApproveToCaller();

        _operatorApprovals[_msgSenderERC721Follower()][operator] = approved;
        emit ApprovalForAll(_msgSenderERC721Follower(), operator, approved);
    }
    ...
    function _transfer(
        ...
        bool isApprovedOrOwner = (_msgSenderERC721Follower() == from ||
            isApprovedForAll(from, _msgSenderERC721Follower()) ||
            approvedAddress == _msgSenderERC721Follower());
    ...
    function _checkContractOnERC721Received(
        ...
        try
            ERC721Follower__IERC721Receiver(to).onERC721Received(
                _msgSenderERC721Follower(),
                from,
                tokenId,
                _data
            )
        returns (bytes4 retval) {
            return
                retval ==
                ERC721Follower__IERC721Receiver(to).onERC721Received.selector;
     ...
     */
    function _msgSenderERC721Follower()
        internal
        view
        virtual
        returns (address)
    ...
    function getAllFund() public view returns (uint256) {
    ...
    // Users get their own FundAmount to be distributed
    function getExtractFundAmount(address userAddress)
        public
        view
        virtual
        returns (uint256)
    {
        ...
        uint256 fundAmount = overage - notWithdrawnProfit;

        // sender's share of FundAmount = royalty profit / total number of issues * number of sender's purchases - the sender's share of FundAmount
        uint256 giveSender = (fundAmount /
            (_currentIndex - _burnCounter - _startTokenId())) *
            balanceOf(userAddress) -
            hadWithdrawedAddress[userAddress];

        // If there is a situation where a late mint user comes in, and the total undrawn amount for all current users < the calculation of the equal share (FundAmount to be shared), it will need to be calculated after subsequent royalty increases
        // This case returns 0
        if (giveSender <= 0) return 0;
        return giveSender;
    }

    function extractFundAmount() public payable {
        uint256 giveSender = getExtractFundAmount(msg.sender);

        if (giveSender == 0) revert NoWithdrawableAmount();

        // Update sender divided FundAmount
